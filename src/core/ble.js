...
export class MxPrinter {
  ...
  async _write(data, onProgress) {
    ...
    const CHUNK = 128;
    // Темп записи = темп печати принтера. Измерено по ae-уведомлениям: при
    // плотности 100% принтер печатает ~40 строк/с (5 мм/с), буфер строк ~4 КБ.
    // 150 мс/пакет ≈ 43 строк/с — чуть быстрее печати: принтер печатает
    // непрерывно (без простоев в паузах), буфер накапливает <1 КБ за задание,
    // переполнения нет, и прогресс по времени совпадает с реальной печатью.
    const PACKET_MS = 150;
