...
export class MxPrinter {
  ...
  async _write(data, onProgress) {
    ...
    const CHUNK = 128;
    // Темп записи подобран под скорость потребления принтером MX11.
    // По замерам: при плотности 100% принтер печатает ~17 мм/с ≈ 2.8 КБ/с
    // (задание 11 КБ ≈ 538 строк, буфер строк ~4 КБ). Писать быстрее нельзя:
    // буфер переполняется, хвост задания теряется, наклейки выходят обрезанными
    // (печаталось 45 мм из 67). 80 мс/пакет ≈ 1.4 КБ/с — почти вдвое ниже
    // потребления, у принтера остаётся запас. Паузы каждые 8 пакетов — страховка.
    const PACKET_MS = 80;
